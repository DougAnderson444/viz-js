# Graphviz wasm component (Wasm Component Model)

The component model is broken down into guest and host. We will be using Rust to host the component and C/C++ to guest the component.

```sh 
make -C packages/viz/component
```

## Installing wasi-sdk

In order to build guest components in C/C++, you will need to install the [WASI SDK] project.
...
Set the environment variable `WASI_SDK_PATH` to point to the extracted
directory, and add its `bin` subdirectory to your `PATH`: 

Move it and create a symlink for easier future upgrades:

```sh 
sudo mv wasi-sdk-29.0-x86_64-linux /opt/
sudo ln -sfn /opt/wasi-sdk-29.0-x86_64-linux /opt/wasi-sdk
...
Thread model: posix
InstalledDir: /opt/wasi-sdk-29.0-x86_64-linux/bin
Configuration file: /opt/wasi-sdk-29.0-x86_64-linux/bin/clang.cfg
```

Alternatively, you can just call clang with explicit sysroot:
